def Catboy():
    ...
    def download():
        ...
        url = 'https://api.catboys.com/img'; response = requests.get(url)

        #Get the JSON response, get the URL from the JSON, Generate the filename, and get the image Data
        data = json.loads(response.content); image_url = data["url"]; 
        filename = os.path.join(os.path.basename(image_url));filename = filename.replace('image','Catboy'); filename = f'Catboy/{filename}'
    ...
    if not os.path.exists('Catboy'):
                os.makedirs('Catboy')
    while True:
        print_centered(f'''
        {Pink}How{Reset} many {Pink}Catboy{Reset} Gif you want to {Pink}download{Reset}
        [{Pink}1{Reset}] {Pink}All{Reset} of them
        [{Pink}2{Reset}] {Pink}Specific{Reset} amount
        [{Pink}3{Reset}] {Pink}Go{Reset} Back''')
        choice = input_centered(f'-> ')
        ...
        if choice == '1':
            ...
            clear_console()
            print_centered(f'{Pink}How{Reset} many {Pink}Catboy{Reset} image you want to {Pink}download{Reset}')
            try:
                a = input_centered('-> ')
                clear_console()
